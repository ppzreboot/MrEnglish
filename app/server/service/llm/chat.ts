...
async function title_chat(client: OpenAI, msg: string): string {
...
async function send_new_msg(app_model: I_app_model, client: OpenAI, chat_id: ObjectId): string {
    // retieve history msg
...
export
async function create_chat(msg: string) {
    const now = new Date()
