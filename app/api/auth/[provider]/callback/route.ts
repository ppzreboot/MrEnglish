...
import { session_manager, get_auth_provider, I_auth_provider_key } from '#service/auth'
...
export async function GET(
	...
	const provider = get_auth_provider(provider_key as I_auth_provider_key)
	if (!provider)
	...
	const cookie_name = `${provider_key}_auth_state`
	...
	const user = await user_service.retrieve_by_provider({
		provider: provider_key as I_auth_provider_key,
		provider_id,
	})
